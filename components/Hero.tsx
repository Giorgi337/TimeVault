...
export const Hero: React.FC = () => {
  ...
  return (
    <section ref={containerRef} className="relative min-h-[100vh] flex flex-col items-center justify-center pt-20 pb-20 md:pt-28 md:pb-24 overflow-hidden bg-slate-950">
      ...
      <motion.div 
        style={{ y: bgTextY }}
        // Adjusted bottom position to make text visible (-10% was too low) but kept low enough to not interfere with content
        className="absolute bottom-[-2%] md:bottom-[-2%] md:landscape:bottom-[-5%] left-0 w-full flex justify-center items-end pointer-events-none select-none z-0 overflow-visible"
      >
        <h2 
          className="text-[15.2vw] md:text-[12vw] md:landscape:text-[10vw] font-black leading-none tracking-[-0.05em] text-blue-500/[0.05] uppercase whitespace-nowrap transition-opacity text-center w-full"
      ...
      <motion.div 
        ...
        className="relative z-20 text-center px-4 md:px-12 max-w-5xl md:max-w-2xl lg:max-w-4xl flex flex-col items-center mt-8 md:-mt-24 md:landscape:mt-16 lg:mt-12"
      >
        <motion.h1 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, ease: "easeOut" }}
          // Reduced font sizes as requested: text-4xl/6xl/7xl (was 5xl/7xl/8xl)
          className={`${isGeorgian ? 'text-xl md:text-4xl lg:text-5xl' : 'text-4xl md:text-6xl lg:text-7xl'} leading-tight ${isGeorgian ? 'tracking-normal' : 'tracking-tighter'} font-black mb-6 md:mb-8 drop-shadow-[0_10px_10px_rgba(0,0,0,0.5)]`}
        >
        ...
        <motion.p 
          ...
          className={`text-lg md:text-2xl max-w-2xl text-slate-300 mb-8 md:mb-10 mx-auto font-medium drop-shadow-lg`}
        >
          {t('hero.description')}
        </motion.p>
        ...
        >
          <BeamButton 
            variant="secondary" 
            className="px-10 py-5 text-lg w-full sm:w-auto shadow-[0_0_40px_rgba(234,179,8,0.3)] hover:shadow-[0_0_60px_rgba(234,179,8,0.5)] transition-all min-w-[200px]"
      ...
      <div className="absolute inset-0 pointer-events-none z-30 hidden md:block">
        <FloatingAsset 
          ...
          className={`top-[25%] left-[15%] md:top-[12%] md:left-[4%] md:landscape:top-[18%] md:landscape:left-[10%] lg:top-[28%] ${isGeorgian ? 'lg:left-[18%]' : 'lg:left-[22%]'}`}
        ...
        <FloatingAsset 
          ...
          className={`top-[20%] right-[15%] md:top-[12%] md:right-[4%] md:landscape:top-[18%] md:landscape:right-[10%] lg:top-[24%] lg:right-[22%]`}
          progress={scrollYProgress}
          speed={-120}
          glowColor="rgba(234, 179, 8, 0.15)"
        />
        <FloatingAsset 
          ...
          className={`bottom-[35%] left-[18%] md:bottom-[15%] md:left-[4%] md:landscape:bottom-[20%] md:landscape:left-[10%] lg:bottom-[38%] lg:left-[25%]`}
          progress={scrollYProgress}
          speed={-450}
        ...
        <FloatingAsset 
          ...
          className={`bottom-[25%] right-[15%] md:bottom-[15%] md:right-[4%] md:landscape:bottom-[20%] md:landscape:right-[10%] lg:bottom-[28%] lg:right-[22%]`}
          progress={scrollYProgress}
          speed={-180}
          glowColor="rgba(234, 179, 8, 0.15)"
        />
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="flex items-center gap-2 bg-slate-900/95 backdrop-blur-2xl p-3.5 rounded-2xl border border-white/30 shadow-2xl hover:border-white/50 hover:bg-slate-800 transition-colors duration-300 group cursor-default pointer-events-auto"
        onClick={(e) => {
            e.stopPropagation(); 
        }}
      >
        {/* Reduced icon container padding (p-2 -> p-1.5) and icon size (20 -> 16 -> 18) */}
        <div className="p-1.5 bg-white/10 rounded-xl group-hover:rotate-12 transition-transform duration-300 shadow-inner">
          {React.cloneElement(icon as React.ReactElement<any>, { size: 18 })}
        </div>
        {/* Reduced text size (text-[13px] -> text-xs -> text-sm) */}
        <span className={`text-sm font-extrabold whitespace-nowrap text-slate-50 group-hover:text-yellow-400 transition-colors tracking-tight`}>{label}</span>
      </motion.div>
    </motion.div>
  );
};